...
namespace lwr_controllers {
    ...
    void CartesianImpedanceBorisController::load_gains() { // carica dal file mat.txt i valori delle matrici per il sistema massa molla smorzatore a ciclo chiuso
		// il file mat.txt contiene (n ordine) i valori della matrice K, di D, di M, le posizioni dei giunti del braccio sinistro e destro di Boris
    ...
    Eigen::Matrix<double,3,3> CartesianImpedanceBorisController::quat_rot(double x, double y, double z, double w) {
    ...
    Eigen::Matrix<double,3,3> CartesianImpedanceBorisController::skew(Eigen::Matrix<double,3,1> v) {
        Eigen::Matrix<double,3,3> temp;
        temp <<     0, -v(2),  v(1),
                 v(2),     0, -v(0),
                -v(1),  v(0),     0;
    ...
    double CartesianImpedanceBorisController::norm(Eigen::VectorXd v) {
    ...
    bool CartesianImpedanceBorisController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n) {
        ...
        sub_command_ = nh_.subscribe("command", 1, &CartesianImpedanceBorisController::command, this);
...
//Compute control law. This controller sets all variables for the JointImpedance Interface from kuka
    ...
    void CartesianImpedanceBorisController::command(const lwr_controllers::PoseRPY::ConstPtr &msg) {	// callback per gestire messaggio della posa desiderata da terminale
...
PLUGINLIB_EXPORT_CLASS( lwr_controllers::CartesianImpedanceBorisController, controller_interface::ControllerBase)
